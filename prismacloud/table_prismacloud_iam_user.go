...
func tablePrismacloudIAMUser(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "prismacloud_iam_user",
		Description: "List all available users and service accounts.",
		List: &plugin.ListConfig{
			Hydrate: listPrismacloudIAMUsers,
		},
...
func listPrismacloudIAMUsers(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	conn, err := connect(ctx, d)
	if err != nil {
		plugin.Logger(ctx).Error("prismacloud_iam_user.listPrismacloudIAMUsers", "connection_error", err)
		return nil, err
	}

	users, err := profile.List(conn)
	if err != nil {
		plugin.Logger(ctx).Error("prismacloud_iam_user.listPrismacloudIAMUsers", "api_error", err)
		return nil, err
	}
